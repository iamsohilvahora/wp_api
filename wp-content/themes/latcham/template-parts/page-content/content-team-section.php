...
if(have_rows('page_flexible_content', $post_id)):
      while(have_rows('page_flexible_content', $post_id)): the_row();
          if( get_row_layout() == 'team_section_page' && $_POST['ajaxIndex'] == get_row_index() ):
            $content_page_team_text = get_sub_field('content_page_team_text');
            $content_page_team_description = get_sub_field('content_page_team_description');
            $content_page_team_button = get_sub_field('content_page_team_button');
            $content_page_team_post = get_sub_field('content_page_team_post');
?>
<div class="container container-align-left">
      <div class="row align-items-center">
          <div class="col col-sm-12 col-lg-6 content">
              <div class="content-inner">
                  <?php if(!empty($content_page_team_text)){ ?>
                  <h2 class="title"><?php echo $content_page_team_text; ?></h2>
                  <?php } if(!empty($content_page_team_description)){ ?>
                  <p><?php echo $content_page_team_description; ?></p>
                  <?php } if(!empty($content_page_team_button)){
                  ?>
                  <div class="button-group">
                      <?php echo button_group($content_page_team_button,'btn btn-primary'); ?>
                  </div>
              <?php } ?>
              </div>
          </div>
          <?php if(!empty($content_page_team_post)){ ?>
          <div class="col col-sm-12 col-lg-6 image team-post-carsoul">
              <div class="team-member-carsoul slide-arrow-top">
                  <?php foreach($content_page_team_post as $team){
