...
if(have_rows('post_flexible_content', $post_id)):
      while(have_rows('post_flexible_content', $post_id)): the_row();
          if( get_row_layout() == 'logo_post_section_post' && $_POST['ajaxIndex'] == get_row_index() ):
          	$logo_post = get_sub_field('content_post_logo_post');
            $logo_post_show_title = get_sub_field('content_post_logo_post_show_title');
            $logo_post_show_text = get_sub_field('content_post_logo_post_show_text');
if(!empty($logo_post)){          
?>            
<div class="container">
