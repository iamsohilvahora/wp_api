...
if(have_rows('case_study_content', $post_id)):
      while(have_rows('case_study_content', $post_id)): the_row();
          if( get_row_layout() == 'team_section' && $_POST['ajaxIndex'] == get_row_index() ):
            $casestudy_team_text = get_sub_field('casestudy_team_text');
            $casestudy_team_description = get_sub_field('casestudy_team_description');
            $casestudy_team_button = get_sub_field('casestudy_team_button');
            $casestudy_team_post = get_sub_field('casestudy_team_post');
?>
<div class="container container-align-left">
      <div class="row align-items-center">
          <div class="col col-sm-12 col-lg-6 content">
              <div class="content-inner">
                  <?php if(!empty($casestudy_team_text)){ ?>
                  <h2 class="title"><?php echo $casestudy_team_text; ?></h2>
                  <?php } if(!empty($casestudy_team_description)){ ?>
                  <p><?php echo $casestudy_team_description; ?></p>
                  <?php } if(!empty($casestudy_team_button)){
                  ?>
                  <div class="button-group">
                      <?php echo button_group($casestudy_team_button,'btn btn-primary'); ?>
                  </div>
              <?php } ?>
              </div>
          </div>
          <?php if(!empty($casestudy_team_post)){ ?>
          <div class="col col-sm-12 col-lg-6 image team-post-carsoul">
              <div class="team-member-carsoul slide-arrow-top">
                  <?php foreach($casestudy_team_post as $team){
