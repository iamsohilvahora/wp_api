...
if(have_rows('page_flexible_content', $post_id)):
      while(have_rows('page_flexible_content', $post_id)): the_row();
          if( get_row_layout() == 'case_studies_section_page' && $_POST['ajaxIndex'] == get_row_index()):
              	$title = get_sub_field('content_page_title');
                $description = get_sub_field('content_page_description');
                $case_studies_btn = get_sub_field('content_page_button');
                ...
                $link_label = get_sub_field('content_page_link_label');
                $inquiry_link = get_sub_field('content_page_inquiry_link');
                $internal_link = get_sub_field('content_page_internal_link');
                $external_link = get_sub_field('content_page_external_link');
                if($inquiry_link == 'Internal'){
                    $link =latcham_external_link($internal_link,false);
                }else{
                    $link =latcham_external_link($external_link,true);
                }        
            $case_studies = get_sub_field('content_page_case_studies');
?>          	
