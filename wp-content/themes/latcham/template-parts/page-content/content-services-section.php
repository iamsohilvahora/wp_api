...
if(have_rows('page_flexible_content', $post_id)):
      while(have_rows('page_flexible_content', $post_id)): the_row();
          if( get_row_layout() == 'services_section_page' && $_POST['ajaxIndex'] == get_row_index()):
              	$heading_title = get_sub_field('content_page_heading_title');
                $services = get_sub_field('content_page_utilities');
                $our_service_button = get_sub_field('content_page_our_service_button');
